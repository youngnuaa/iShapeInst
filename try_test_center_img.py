...
def draw_img(result, img):
    ...
    #pred_masks = result.pred_masks.tensor
    pred_masks = result.pred_masks
    center_indexs = result.center_index

    # print(pred_masks)
    num, _, _ = pred_masks.shape
    colors = random_colors(num)

    #img = img * 0.7 + 255 * 0.3

    img_path = "./img/" + str(0) + "ori_img.jpg"
    cv2.imwrite(img_path, img)


    for index in range(num):
        mask_img = img.copy()
        center_img = img.copy() * 0.7 + 255 * 0.3

        pred_mask = pred_masks[index]
        center_index = center_indexs[index]

        pred_mask = pred_mask.cpu().numpy()
        center_index = center_index.cpu().numpy()

        c_y = int((center_index // 68+0.5)*15.0588)
        c_x = int((center_index % 68+0.5)*15.0588)

        edge_img = draw_instance_edge(pred_mask)
        c_mask = draw_center_point(pred_mask, (c_x, c_y))
        ...
        print("object score", pred_scores[index])
        print(index)
        color = colors[index]
        color = [80, 30, 180]
        ...
        #img[mask == 1] = img[mask == 1] * 0.5 + rgba[mask == 1] * 0.5
        #new_img[mask == 1] = new_img[mask == 1] * 0.5 + rgba[mask == 1] * 0.5

        #img[edge_img == 1] = img[edge_img == 1] * 0.0 + rgba[edge_img == 1] * 1.0
        #new_img[edge_img == 1] = new_img[edge_img == 1] * 0.0 + rgba[edge_img == 1] * 1.0

        #mask_img
        #img[edge_img == 1] = img[edge_img == 1] * 0.0 + rgba[edge_img == 1] * 0
        mask_img[edge_img == 1] = mask_img[edge_img == 1] * 0.0 + rgba[edge_img == 1] * 0
        mask_img[mask > 0.05] = mask_img[mask > 0.05] * 0.35 + rgba[mask > 0.05] * 0.65
        img_path = "./img/" + str(index) + "mask_img.jpg"
        cv2.imwrite(img_path, mask_img)

        #center_img
        #img[c_mask > 0.05] = img[c_mask > 0.05] * 0.5 + rgba[c_mask > 0.05] * 0.5
        center_img[c_mask > 0.05] = center_img[c_mask > 0.05] * 0.2 + rgba[c_mask > 0.05] * 0.8
        #center_img[edge_img == 1] = center_img[edge_img == 1] * 0.0 + rgba[edge_img == 1] * 0

        img_path = "./img/" + str(index) + "center_img.jpg"
        cv2.imwrite(img_path, center_img)
        print("end")
